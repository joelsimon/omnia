...
%
% Calculated location of points lying on a great circle through a
% certain
% point with a certain azimuth at a certain epicentral
% distance. One of
...
if ~isstr(lonlat)
    ...
    angl=angl*pi/180;
    % Only one vector at a time except if lonlat and dlta have the same
    % length
    if (length(lat1)==1 & length(dlta)==1) | (length(lat1)==1 & ...
                                              length(angl)==1)
        lat2 =asin(sin(lat1)*cos(dlta)+cos(lat1)*sin(dlta)*cos(angl));
        dlon=atan2(sin(angl)*sin(dlta)*cos(lat1),cos(dlta)-sin(lat1)* ...
                   sin(lat2));
    ...
    elseif length(angl)==1
        lat2=asin(sin(lat1)*cos(dlta(:)')+cos(lat1)*sin(dlta(:)')* ...
                  cos(angl));
        dlon=atan2(cos(lat1)*sin(angl)*sin(dlta(:)'),...
