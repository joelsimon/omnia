% Modified FIGURE 4 using rvar, the ratio of sample variances, for the
% reviewer.
%
% Cite: Simon, J. D. et al., (2020), BSSA, doi: 10.1785/0120190173
%
% Plots the theoretical summed log-likelihood value (of an ideal
% time series with normlysmixvals.m), and actual summed log-likelihood
...
for i = 1:25
    ...
    plest{i} = plot(ax, est_rvar, 'Color', [0.6 0.6 0.6]);
    ave_est_rvar = [ave_est_rvar + est_rvar];
end

% Plot the average of the estimated summed log-likelihoods -- should
% look a lot like true, theoretical ones below (tru_sumly).
ave_est_rvar = ave_est_rvar / 25;

plave = plot(ax, ave_est_rvar, 'k', 'LineWidth', 2);
...
ko = text(ax, 500, ax.YLim(2)+.1, '$k_{\circ}$');
